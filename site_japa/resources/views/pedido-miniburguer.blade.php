      ...
      <div class="col-sm-9">
        <div class="row">
          ...
          <form role="form" method="post" action=" {{ action('CompraController@addProdutoCarrinho') }}">
            ...
            <div class="card-deck" style="width: 21rem; height: 40rem; margin-bottom: 10px">
  <div class="card">
                <img class="card-img-top" src="{{ url("/img/$e->img_produto") }}" alt="Card image cap">
    <div class="card-body">
      
                  <p style="font-size: 25px" class="card-title">{{ $e->sabor }} </p>
                  <?php $valor_formatado = number_format($e->preco, 2, ',', '.'); ?>
                  <p style="font-size:21px" class="card-text">Combo:  <b style="color: green"> R$ {{ $valor_formatado }} </b></p>
                  <p id="fonteIngre" style="margin-top:-10px; font-size:15px" class="card-text">Acompanha: {{ $e->ingredientes }}</p>
    </div>
        ...
        <div class="card">
          <h5 id="fonteMeuPedido" class="card-header" style="background:red; font-size: 2vw"><i class="fa fa-file-text-o" aria-hidden="true"></i>
            ...
            <div class="card-body">
              ...
              <div class="row" style="font-family: 'Signika', sans-serif; font-size:1.2vw;">
                <div class="col-sm-4">
                  <a href="{{ action('CompraController@excluirHossomaki', ['id' => $c->id])}}"> <i style="margin-left:15px; font-size:35px; color: black" class="fa fa-trash-o" aria-hidden="true"></i> </a>
                </div>
                <div class="col-8">
    ...
    <form role="form" method="post" action=" {{ action('CompraController@addProdutoCarrinhoMob') }}">
      ...
      <div class="col-sm-3">
        <div class="card" style="width: 100%;margin-bottom:15px">
          ...
          <div class="card-body">
            <p style="font-size: 27px" class="card-title">{{ $e->sabor }} </p>
            <?php $valor_formatado = number_format($e->preco, 2, ',', '.'); ?>
            <p style="margin-top:-15px; font-size:22px" class="card-text">Combo: <b style="color: green"> R$ {{ $valor_formatado }} </b></p>
            <p id="fonteIngre" style="margin-top:-10px; font-size:18px" class="card-text">Acompanha: {{ $e->ingredientes }}</p>
            <select id="options" name="qtd" class="form-control" required>
              <option value="" disabled="true" selected="true"> - Selecione a quantidade -  </option>
              <option value="1">1</option>
              <option value="2">2</option>
